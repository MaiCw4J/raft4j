...
public class $ {
    ...
    public static boolean isContinuousEntries(List<Eraftpb.Entry> msgEntries, List<Eraftpb.Entry.Builder> entries) {
    ...
    public static Long limitBuilderSize(Vec<Eraftpb.Entry.Builder> entries, Long max) {
        if (entries.size() <= 1 || max == null) {
            return null;
        }
        AtomicLong size = new AtomicLong(0);
        long limit = entries.stream()
                .takeWhile(e -> size.addAndGet(e.build().getSerializedSize()) <= max)
                .count();
